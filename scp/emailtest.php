...
if($_POST){
    ...
    if(!$errors && $email){
        if($email->send($_POST['email'],$_POST['subj'],$_POST['message']))
            $msg=sprintf(__('Test email sent successfully to %s'), Format::htmlchars($_POST['email']));
        else
...
<form action="emailtest.php" method="post" id="save">
 ...
 <table class="form_table" width="940" border="0" cellspacing="0" cellpadding="2">
    <thead>
        <tr>
            <th colspan="2">
                <em><?php echo __('Emails delivery depends on your server settings (php.ini) and/or email SMTP configuration.');?></em>
            </th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr>
            <td colspan=2>
                <em><strong><?php echo __('Message');?></strong>: <?php echo __('email message to send.');?></em>&nbsp;<span class="error">*&nbsp;<?php echo $errors['message']; ?></span><br>
