...
class Mcrypt {
    ...
    function encrypt($text, $salt) {
        ...
        if(!function_exists('mcrypt_encrypt') || !function_exists('mcrypt_decrypt')) {
            if($ost) {
                $msg=__('Cryptography extension mcrypt is not enabled or installed. Important text/data is being stored as plain text in database.');
                $ost->logWarning(__('mcrypt module missing'), $msg);
            }
